...
export default function EditKontenPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/konten/${id}`).then(r => r.json()),
      ...
      setForm({
        tanggal_produksi: konten.tanggal_produksi?.slice(0, 10) ?? '',
        tanggal_tayang: konten.tanggal_tayang?.slice(0, 10) ?? '',
        akun_id: konten.akun_id ? String(konten.akun_id) : '',
        jenis_konten_id: konten.jenis_konten_id ? String(konten.jenis_konten_id) : '',
        tipe_konten_id: konten.tipe_konten_id ? String(konten.tipe_konten_id) : '',
        pic_id: konten.pic_id ? String(konten.pic_id) : '',
        catatan: konten.catatan ?? '',
        referensi_konten: konten.referensi_konten ?? '',
        materi: konten.materi ?? '',
      });
      setPlatformIds(Array.isArray(konten.platform_ids) ? konten.platform_ids : []);
      setTargetInsightIds((konten.target_insights ?? []).map((t: { id: number }) => t.id));
      setLokasi({
        tipe_lokasi: konten.tipe_lokasi ?? '',
        lokasi_kota: konten.lokasi_kota ?? '',
        properti_ids: (konten.properti ?? []).map((p: { id: number }) => p.id),
      });
      setReady(true);
  ...
  function toggleTargetInsight(tid: number) {
    setTargetInsightIds(prev => prev.includes(tid) ? prev.filter(x => x !== tid) : [...prev, tid]);
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-7">
        <button onClick={() => router.back()} className="inline-flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600 mb-3">
